...
fuga<-read.csv2("//nas//Datos//RespaldoPC//IVECLE01//Diplomado Big Data//Indices de forma otolitos Reineta.csv",head=T)
...
qplot(Area, Diameter..max., data=fuga, geom=c("boxplot", "jitter"), 
      fill=Area, main="Area por Diametro Otolito x grupo",
      xlab="", ylab="Area", group=Sexo)
...
fuga[, "Fecha.Vigencia"] <- as.Date(as.character(fuga[, "Fecha.Vigencia"]),format = "%d-%m-%Y")
str(fuga)
...
(prop<-data.frame(prop.table(table(fuga_train$Sexo))))

prop.table(table(fuga_test$Sexo))
#prop.table(table(fuga_validacion$Sexo))
prop.table(table(fuga$Sexo))
...
ir.tune<-tune(svm, Sexo ~., data=fuga_train, 
...
PRUEBA <- svm(Churn ~., data = fuga_train, cross = 5, gamma= ir.tune$best.parameters[,1], cost=ir.tune$best.parameters[,1], kernel= "linear",method="C-classification", probability=T)
...
write.table(prob, "//nas//Datos//RespaldoPC//IVECLE01//1. Fugados 2016//Fugados PAC Enero 2017//Probabilidades//Indicador-Global-histórico.txt")
write.table(prob2, "//nas//Datos//RespaldoPC//IVECLE01//1. Fugados 2016//Fugados PAC Enero 2017//Probabilidades//Probabilidad-Global-histórico.txt")
...
#Guardar probabilidades en un excel
write.table(prob, "//nas//Datos//RespaldoPC//IVECLE01//1. Fugados 2016//Fugados PAC Enero 2017//Probabilidades//Indicador-Global-DatosNuevos.txt")
write.table(prob2, "//nas//Datos//RespaldoPC//IVECLE01//1. Fugados 2016//Fugados PAC Enero 2017//Probabilidades//Probabilidad-Global-DatosNuevos.txt")
