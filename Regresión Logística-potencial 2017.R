...
#[1] "Nro..Cobrador"           "Nombre.Centralizador"    "Mes.Alta.Cobrador"      
#[4] "Mes.Prima.Prox..Cobrza." "Cant..Meses"             "Oficina"                
#[7] "Grupo.Cobranza.Cobrador" "Comision.Cobranza"       "Medio.Magnetico"        
#[10] "Categor.a"               "Medio.de.Pago.Cobrador"  "Paga"                   
#[13] "Condici.n.de.Pago"       "Morosidad"               "Suspende.Descuentos"    
#[16] "Sector"                  "Actividad"               "Clas..Meses.Convenio"   
#[19] "Cant..Trabajadores"      "Cant..Asegurados.P.C."   "Producidas"             
#[22] "Anuladas"                "Cant..Aseg.201703"       "Cant..Aseg.201704"      
#[25] "Cant..Aseg.201705"       "Cant..Aseg.201706"       "rec..75..201703"        
#[28] "rec..75..201704"         "rec..75..201705"         "Acepta.Producci.n"      
...
modelo<-glm(Estado.Oficial ~ .,data = fuga ,family = "binomial")
...
modelo<-glm(formula = Estado.Oficial ~ Condición.de.Pago + Morosidad + 
...
#cor(modelo$fitted.values,Acepta.Producci.n)^2
summary(modelo)
...
plot(prob)
df1<-data.frame(prob)
write.csv2(df1, "//nas//Datos//Liquidacion//Liquidacion 1//PAMELA//75%//1. Análisis 75% (Período Junio 2017)//Prob.csv")
...
pred<-prediction(test$score,test$Estado.Oficial)
...
m1_pred <- prediction(test$m1_score, test$s)
m1_perf <- performance(m1_pred,"tpr","fpr")


#3 razones por la que rechazar las aperturas#
...
m1_h <- CVbinary(obj=m, rand=NULL, nfolds=10, print.details=TRUE)
...
# MODELO 
#------------------------------------------------------------------------------
modelo.rpart  <- rpart(Estado.Oficial ~ .,Train, method="class")
#modelo1<-glm(Estado.Oficial ~ factor(Cod.Edad < 3) + factor(Cod.Mes.Activacion < 2)
#            + Cod.Mes.Inter +  Cod.dif.prima ,data= Train,family = "binomial")
#modelo2<-glm(Estado.Oficial ~ factor(Cod.Edad < 3) + factor(Cod.Mes.Activacion < 2)
#            + Cod.Mes.Inter +  Cod.dif.prima ,data= Test,family = "binomial")
summary(modelo1)
summary(modelo2)
library(ROCR)
...
predict.rocr  <- prediction(predict.rpart,Test$Estado.Oficial)
